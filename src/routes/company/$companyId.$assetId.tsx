...
function AssetsSection() {
  ...
  const component = useAppSelector(
    (state) => state.companyTree.componentById,
    shallowEqual
  );
  ...
  return (
    <section className="w-full h-[80vh] border border-gray-200 border-solid rounded-sm flex flex-col">
      <div className="px-4 py-3 border-b border-solid border-gray-200 flex flex-row items-center gap-2">
        <h2 className="font-semibold text-lg text-gray-950">
          {component?.name}
        </h2>
        <SensorType
          status={component?.status ? component.status : null}
          sensorType={component?.sensorType ? component.sensorType : null}
        />
      </div>
          ...
          <div className="flex flex-col justify-around w-full">
            ...
            <div className="flex flex-col gap-2">
              ...
              <p className="text-gray-600 opacity-90 ">
                <b className="bg-blue-500 px-[10px] py-1 rounded-full mr-2 text-white">
                  E
                </b>
                [RESPONSÁVEL]
              </p>
